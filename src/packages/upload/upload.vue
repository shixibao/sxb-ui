<template>
  <div class="s-upload">
    ...
    <template v-else>
      <div @click="handleClick" class="s-upload-btn">
        <slot></slot>
      </div>
      <input
        type="file"
...
export default {
  ...
  methods: {
    ...
    post(rawFile) {
      ...
      const uid = rawFile.uid; // 这里可能稍后上传的时候 会后悔 中断ajax 序号
      const options = {
        file: rawFile, // 源文件

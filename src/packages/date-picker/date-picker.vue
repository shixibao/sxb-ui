<template>
  <div class="s-date-picker" v-click-outside="handleBlur">
    <s-input suffix-icon="rili" @focus="handleFocus" :value="formateDate" @change="handleChange"></s-input>
    <div class="s-date-content" v-if="isVisible">
      <div class="s-date-picker-content">
        <template v-if="mode === 'dates'">
          <div class="s-date-picker-header">
            <s-icon icon="prev" @click="changeYear(-1)"></s-icon>
            <s-icon icon="left" @click="changeMonth(-1)"></s-icon>
            <span>
              <b @click="mode='years'">{{tempTime.year}}</b>年
              <b @click="mode='months'">{{tempTime.month + 1}}</b>月
            </span>
            <s-icon icon="right" @click="changeMonth(1)"></s-icon>
            <s-icon icon="next" @click="changeYear(1)"></s-icon>
          </div>
          <div>
        ...
        <template v-if="mode === 'years'">
            <div class="s-date-picker-header">
            <s-icon icon="prev" @click="changeYear(-10)"></s-icon>
            <span>
              <b @click="mode='years'">{{startYear}}年{{startYear+9}}年</b>
            </span>
            <s-icon icon="next" @click="changeYear(10)"></s-icon>
          </div>
        </template>
        <template v-if="mode === 'months'">
            <div class="s-date-picker-header">
            <s-icon icon="left" @click="changeYear(-1)"></s-icon>
            <span>
              <b @click="mode='years'">{{this.tempTime.year}}年</b>
            </span>
            <s-icon icon="right" @click="changeYear(1)"></s-icon>
          </div>
        </template>
      </div>
    </div>
